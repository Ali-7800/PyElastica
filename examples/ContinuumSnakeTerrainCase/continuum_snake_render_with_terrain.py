...
included = ["default.inc","surface.inc"]
...
if __name__ == "__main__":
    ...
    base_radius = np.arctan(np.arange(0,51,1)*np.ones_like(xs[:, 0, :])/4)/np.arctan(51/4) * 0.011 *0.8 #snake radial profile to distinguish tail from head
    # Rendering
    ...
    for frame_number in tqdm(range(total_frame), desc="Scripting"):
        for view_name, stage_script in stage_scripts.items():
            ...
            rod_object = pyelastica_rod(
                ...
                color="rgb<20/255,14/255,14/255>",
            )
