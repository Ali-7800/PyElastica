...
def compute_effective_velocity(plot_params: dict, period):
    ...
    # Select the number of period to start the computing - ignore the first several periods to 
    # get the steady speed
    start_period = 2              # first period = 1

    # Center of mass position averaged in one period
    center_of_mass_averaged_over_one_period = np.zeros((number_of_period - start_period, 3))
    for i in range(0, number_of_period - start_period):
        # position of center of mass averaged over one period
        center_of_mass_averaged_over_one_period[i] = np.mean(
            center_of_mass[(i + start_period) * period_step : (i + start_period + 1) * period_step]
            - center_of_mass[(i + start_period - 1) * period_step : (i + start_period) * period_step],
            axis=0,
        )

    effective_velocity_over_one_period = np.linalg.norm(center_of_mass_averaged_over_one_period, axis=1)
    effective_velocity = np.mean(effective_velocity_over_one_period)
