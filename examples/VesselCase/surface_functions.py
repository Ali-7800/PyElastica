    ...
    **kwargs,
    ...
    if "up_direction" in kwargs.keys():
        for i in range(n_facets):
            ...
            facets_centers[:,i] = np.mean(facets[:,:,i],axis=1) #center of the facet vertices
            #find the normal vector by taking the cross product using two facet sides
            facets_normals[:,i] = np.cross(facets[:,2,i]-facets[:,1,i],facets[:,2,i]-facets[:,0,i]) 
            ...
            dotproduct = np.dot(up_direction, facets_normals[:,i]) #check normal vector orientation with respect to the up_direction
            if dotproduct<0:
                facets_normals[:,i] = -facets_normals[:,i] 
    else:
        facet_vertex_normals = kwargs["facet_vertex_normals"]
        assert (facet_vertex_normals.shape[-1] == n_facets), "Number of facets in facet_vertex_normals is not equal to the number of facets"
        for i in range(n_facets):
            facets_centers[:,i] = np.mean(facets[:,:,i],axis=1) #center of the facet vertices
            #find the normal vector by taking the cross product using two facet sides
            facets_normals[:,i] = np.cross(facets[:,2,i]-facets[:,1,i],facets[:,2,i]-facets[:,0,i]) 
            face_areas[i] = np.linalg.norm(facets_normals[:,i])
            if np.linalg.norm(facets_normals[:,i]) < 1e-16: #if normal has zero magintude set it to point up
                facets_normals[:,i] = facet_vertex_normals[:,0,i]
            else:
                facets_normals[:,i] /= face_areas[i]
            dotproduct = np.dot(facet_vertex_normals[:,0,i], facets_normals[:,i]) #check normal vector orientation with respect to one of the vertex normals
            if dotproduct<0:
                facets_normals[:,i] = -facets_normals[:,i] 
...
def import_surface_from_obj(
    ...
    for line in surface_model_text.splitlines():
        if len(line)>0:
            if line[0] == 'v' and line[1] == ' ':
                ...
                facet_texture_index.append(n_texture_file-1) #include this to figure out where each texture this face is mapped to
    ...
    mesh_center = np.array([np.max(vertices[:,0])+np.min(vertices[:,0]),np.max(vertices[:,1])+np.min(vertices[:,1]),np.max(vertices[:,2])+np.min(vertices[:,2])])/2
    vertices -= mesh_center
    #set maximum z value
    vertices[:,2] += -np.max(vertices[:,2]) + max_z 
    print(np.array([np.max(vertices[:,0])+np.min(vertices[:,0]),np.max(vertices[:,1])+np.min(vertices[:,1]),np.max(vertices[:,2])+np.min(vertices[:,2])])/2
)
    ...
    if povray_viz:
        ...
        for i in range(n_vertices):
            mesh_list.append(",<"+str(vertices[i,0])+","+str(vertices[i,1])+","+str(vertices[i,2])+">")
        mesh_list.append("\n}")
            
        mesh_list.append("\nnormal_vectors {")
        mesh_list.append("\n" + str(n_vertices_normal))
        for i in range(n_vertices_normal):
            mesh_list.append(",<"+str(vertices_normals[i,0])+","+str(vertices_normals[i,1])+","+str(vertices_normals[i,2])+">")
        mesh_list.append("\n}")

        mesh_list.append("\nuv_vectors {")
        mesh_list.append("\n" + str(n_texture_vertices))
        for i in range(n_texture_vertices):
            mesh_list.append(",<"+str(texture_vertices[i,0])+","+str(texture_vertices[i,1])+">")
        mesh_list.append("\n}")
        ...
        if ~with_texture:
            ...
            for i in range(n_texture_file):
                mesh_list.append("\ntexture {\npigment {\nuv_mapping \nimage_map {\njpeg \"" + model_path + "/"+str(mtl_dict[i+1])+"_diffuse.jpg\" \nmap_type 0\n}\n}") #add color
                if os.path.exists(model_path + "/"+str(mtl_dict[i+1])+"_normal.jpg"):
                    mesh_list.append("\nnormal {\nuv_mapping \nbump_map { \njpeg \"" + model_path + "/"+str(mtl_dict[i+1])+"_normal.jpg\" \nmap_type 0\nbump_size 50\n}\n}") #add normals
                mesh_list.append("\n}")
        mesh_list.append("\n}")
        ...
        mesh_list.append("\n" + str(n_faces))
        for i in range(n_faces):
            mesh_list.append(",<"+str(faces[i,0,0])+","+str(faces[i,1,0])+","+str(faces[i,2,0])+">,"+str(facet_texture_index[i]))
        mesh_list.append("\n}")

        mesh_list.append("\nuv_indices {")
        mesh_list.append("\n" + str(n_faces))
        for i in range(n_faces):
            mesh_list.append(",<"+str(faces[i,0,1])+","+str(faces[i,1,1])+","+str(faces[i,2,1])+">")
        mesh_list.append("\n}")

        mesh_list.append("\nnormal_indices {")
        mesh_list.append("\n" + str(n_faces))
        for i in range(n_faces):
            mesh_list.append(",<"+str(faces[i,0,2])+","+str(faces[i,1,2])+","+str(faces[i,2,2])+">")
        mesh_list.append("\n}")
...
def create_surface_from_parameterization(
    ...
    if povray_viz:
        ...
        for i in range(n_facets):
            ...
            color_list.append("#declare Color" + str(i) + " = texture {\n pigment { color rgb<" + str(color[0]) +"," + str(color[1]) + "," + str(color[2])+"> }\nfinish { phong 0.2 }}\n")
...
def surface_grid_numba(
    ...
    faces_grid = dict()
    for i in range(n_x_positions):
        x_left = x_min + (max(0,i-1) * grid_size)  # current grid square left x coordinate
        x_right = x_min + (min(n_x_positions-1,i + 1) * grid_size)  # current grid square right x coordinate
        for j in range(n_y_positions):
            y_down = y_min + (max(0,j-1) * grid_size)  # current grid square down y coordinate
            y_up = y_min + (min(n_y_positions-1,j + 1) * grid_size)  # current grid square up y coordinate
            if (
                len(
                    np.where(
                        (
...
def surface_grid(faces, grid_size):
    ...
    print("Creating Grid ...")
    facets_grid = dict(surface_grid_numba(faces, grid_size, face_x_left, face_x_right, face_y_down, face_y_up))
    print("Grid Created!")
