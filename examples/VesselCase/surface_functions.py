...
def import_surface_from_obj(
    ...
    texture_exists = os.path.exists(model_path+"/texture.jpg")
    normal_exists = os.path.exists(model_path+"/normal.jpg")
    ...
    if povray_viz:
        ...
        mesh_list.append("\n" + str(n_faces))
        for i in range(n_faces):
            mesh_list.append(",<"+str(faces[i,0,0])+","+str(faces[i,1,0])+","+str(faces[i,2,0])+">")
        mesh_list.append("\n}")
        ...
        if texture_exists:
            mesh_list.append("\ntexture {\npigment {\nuv_mapping \nimage_map {\njpeg \"" + model_path + "/texture.jpg\" \nmap_type 0\n}\n}") #add color
            if normal_exists:
                mesh_list.append("\nnormal {\nuv_mapping \nbump_map { \njpeg \"" + model_path + "/normal.jpg\" \nmap_type 0\nbump_size 50\n}\n}") #add normals
            mesh_list.append("\n}\n}")
        else:
...
@njit(cache=True)
def surface_grid_numba(facets,grid_size,facet_x_left,facet_x_right,facet_y_down,facet_y_up):
    ...
    for i in range(n_x_positions):
        ...
        for j in range(n_y_positions):
            ...
            if np.any(np.where(((facet_y_down>y_up) + (facet_y_up<y_down) + (facet_x_right<x_left) + (facet_x_left>x_right))==0)[0]):
                facets_grid[(i,j)] = np.where(((facet_y_down>y_up) + (facet_y_up<y_down) + (facet_x_right<x_left) + (facet_x_left>x_right))==0)[0]
    return facets_grid
...
def surface_grid(facets,grid_size):
    ...
    print("Creating Grid ...")
    facets_grid = dict(surface_grid_numba(facets,grid_size,facet_x_left,facet_x_right,facet_y_down,facet_y_up))
    print("Grid Created!")
