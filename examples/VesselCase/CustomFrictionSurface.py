...
def find_contact_facets_idx(
    ...
    for i in range(n_element):
        ...
        try:
            facet_idx_element_no_duplicates = facets_grid[(int(grid_position[0,i]),int(grid_position[1,i]))]
        except:
...
class CustomInteractionSurfaceGrid:
    ...
    def __init__(self, k,nu,facets,facets_grid,facets_normals,facets_centers,side_vectors,grid_size):
        """
...
def apply_normal_force_numba(
    ...
    projected_element_position = center_to_center_vector - distance_from_facet_plane*normals_on_elements  
    plane_intersection_radius = np.sqrt(np.maximum(radius_contacts**2-distance_from_facet_plane**2,0)) #using pythagoras, figure out the intersection radius (we can modify this later for cylinder)
    ...
    closest_point_on_triangle_to_projected_point[:,region_1] = contact_facets_A[:,region_1]
    region_2 = np.where((d3 >= 0)*(d4 <= d3))[0] #region 2 (closest to vertex B)
    closest_point_on_triangle_to_projected_point[:,region_2] = contact_facets_B[:,region_2]
    region_3 = np.where((d6 >= 0)*(d5 <= d6))[0] #region 3 (closest to vertex C)
    closest_point_on_triangle_to_projected_point[:,region_3] = contact_facets_C[:,region_3]
    region_4 = np.where((vc <= 0)*(d1 >= 0)*(d3 <= 0))[0] #region 4 (closest to some point on AB)
    closest_point_on_triangle_to_projected_point[:,region_4] = contact_facets_A[:,region_4] + d1[region_4]*contact_side_AB[:,region_4]/(d1[region_4]-d3[region_4])
    region_5 = np.where((vb <= 0)*(d2 >= 0)*(d6 <= 0))[0] #region 5 (closest to some point on AC)
    closest_point_on_triangle_to_projected_point[:,region_5] = contact_facets_A[:,region_5] + d2[region_5]*contact_side_AC[:,region_5]/(d2[region_5]-d6[region_5])
    region_6 = np.where((va <= 0)*(d4 >= d3)*(d5 >= d6))[0] #region 6 (closest to some point on BC)
    closest_point_on_triangle_to_projected_point[:,region_6] = contact_facets_B[:,region_6] + (d4[region_6]-d3[region_6])*contact_side_BC[:,region_6]/((d4[region_6]-d3[region_6])+(d5[region_6]-d6[region_6]))
    #otherwise it is inside the triangle hence closest point will be the same as the projected position
    distance_to_closest_triangle_point = _batch_norm(closest_point_on_triangle_to_projected_point-projected_element_position)
    ...
    no_penetration_idx = np.where((distance_from_facet_plane - radius_contacts) > surface_tol)[0]
    ...
    no_intersection_idx = np.where((distance_to_closest_triangle_point - plane_intersection_radius) > surface_tol)[0]  
    ...
    return _batch_norm(plane_response_force_contacts),no_penetration_idx,no_intersection_idx,normals_on_elements
...
class CustomFrictionalSurfaceGrid(NoForces, CustomInteractionSurfaceGrid):
    def __init__(
        ...
        """
        CustomInteractionSurfaceGrid.__init__(self, k, nu, facets,facets_grid,facets_normals,facets_centers,side_vectors, grid_size)
    ...
    def make_callback(self):
        ...
        if self.current_step % self.every == 0:
            self.callback_params['facet_idx_array'].append(
                self.facet_idx_array.copy()
            )
...
def anisotropic_friction(
    ...
    plane_response_force_mag,no_penetration_idx,no_intersection_idx,normals_on_elements = apply_normal_force_numba(
        facets,
        facets_normals,
        facets_centers,
        element_position,
    ...
    if len(position_idx_array)>0:
        ...
        kinetic_mu_sideways_array = kinetic_mu_sideways*np.ones_like(position_idx_array,dtype=np.float64)
        elements_within_pattern_idx = np.where(facet_pattern_idx[facet_idx_array])[0] #this checks which facets that are within the pattern
        kinetic_mu_sideways_array[elements_within_pattern_idx] = kinetic_mu_sideways_pattern
    else:
    ...
    kinetic_friction_force_along_axial_direction_contacts[..., no_intersection_idx] = 0.0
    ...
    # If rod element does not have any contact with plane, plane cannot apply friction
    # force on the element. Thus lets set kinetic friction force to 0.0 for the no contact points.
    kinetic_friction_force_along_rolling_direction_contacts[..., no_penetration_idx] = 0.0
    kinetic_friction_force_along_rolling_direction_contacts[..., no_intersection_idx] = 0.0
    ...
    static_friction_force_along_rolling_direction_contacts[..., no_penetration_idx] = 0.0
