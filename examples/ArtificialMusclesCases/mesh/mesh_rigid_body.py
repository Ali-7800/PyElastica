...
class MeshRigidBody(RigidBodyBase):
    def __init__(
        ...
        for k in range(self.n_faces):
            for l in range(3):
                for i in range(3):
                    test_faces[i, l, k] += center_of_mass[i]
                    for j in range(3):
                        test_faces[i, l, k] += (
                            self.distance_to_faces[l, k]
                            * self.director_collection[i, j, 0]
                            * self.direction_to_faces[j, l, k]
                        )

        print(np.linalg.norm(test_faces - self.faces))
...
def _update_faces(
    ...
    for k in range(n_faces):
        for l in range(3):
            for i in range(3):
                faces[i, l, k] += center_of_mass[i, 0]
                for j in range(3):
                    faces[i, l, k] += (
                        distance_to_faces[l, k]
                        * director_collection[i, j, 0]
                        * direction_to_faces[j, l, k]
                    )
