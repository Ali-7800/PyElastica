...
def muscle_stretching_actuation_simulator_multicycle(
    ...
    for cycle in range(forcing_settings.n_cycles):
        force_scale = (
            1e-3
            * forcing_settings.force_mag_func(cycle, forcing_settings.default_force_mag)
        ...
        print(
            "Cycle: " + str(cycle),
            "Force: " + str(end_force * input_muscle.sim_settings.E_scale),
        )
        forcing_duration = forcing_settings.forcing_duration_func(cycle)

        cycle_start_time = forcing_settings.forcing_start_time + cycle * (
            forcing_settings.forcing_duration_func(cycle - 1)
            + forcing_settings.rest_duration
        )
        ...
        if forcing_settings.desired_muscle_strain > 0:
            muscle.add_forcing_to_muscle(
                ...
                ramp_down_time=forcing_settings.ramp_up_time,
                start_time=cycle_start_time,
                end_time=cycle_start_time + forcing_duration,
            )
    ...
    if sim_settings.plot_video:
        if constrain_settings.isometric_test == True:
            ...
            filename_video = (
                sim_settings.sim_name
                + "_force_mag_"
                + str(forcing_settings.default_force_mag)
                + "_"
                + input_muscle.name
                + "_"
                + sim_settings.additional_identifier
