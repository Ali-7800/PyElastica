...
from examples.OctopusArmControl._rod_tool import (
    _lab_to_material,
    _material_to_lab,
    average2D,
)
...
class ApplyActuations(NoForces):
    ...
    def apply_torques(self, system, time: np.float64 = 0.0):
        ...
        for actuation in self.actuations:
            actuation(system)
            inplace_addition(system.external_forces, actuation.external_force)
            inplace_addition(system.external_torques, actuation.external_couple)
    ...
    def make_callback(self):
        ...
        if self.current_step % self.every == 0:
            self.callback_func(self.actuations, self.callback_params_list)
        self.current_step += 1
    ...
    def callback_func(self, actuations, callback_params_list):
        ...
        """
        for actuation, callback_params in zip(actuations, callback_params_list):
            callback_params["internal_force"].append(actuation.internal_force.copy())
            callback_params["internal_couple"].append(actuation.internal_couple.copy())
            callback_params["external_force"].append(actuation.external_forces.copy())
            callback_params["external_couple"].append(actuation.external_couple.copy())
