...
import numpy as np
from examples.OctopusArmControl._rod_tool import _material_to_lab
...
class POVRAYSlenderMuscle(POVRAYMuscle):
    def __init__(self, **kwargs):
        ...
        POVRAYMuscle.__init__(
            self, muscle_color, kwargs.get("activation_color", muscle_color)
        )
    ...
    def write_to(
        ...
        position_data,
        director_data,
        muscle_position_data,
        radius_data,
        muscle_activation=None,
        alpha=1.0,
    ):
    ...
    def write_to(
        ...
        for n in range(n_elements - 1):
            ...
            for nn in range(self.n_muscle_nodes + 1):
                ...
                position_muscle = position[:, n] + (
                    major_radius[n] - minor_radius[n]
                ) * (
                    np.cos(theta) * director_data[0, :, n]
                    + np.sin(theta) * director_data[1, :, n]
                )
                string += ",\n\t<%f, %f, %f>, %f" % (
                    position_muscle[0],
                    position_muscle[1],
